...
int main()
{
	try {
		...
		// test credit: Ava Jih-Schiff
		{
			...
			test.execute( Push { "abc" }.with_close() );
			test.execute(
			  ExpectMessage {}.with_no_flags().with_syn( true ).with_payload_size( 2 ).with_seqno( isn ) );
		...
		// test credit: Majd Nasra
		{
			...
			test.execute(
			  ExpectMessage {}.with_data( "01234567" ).with_syn( true ).with_seqno( isn ).with_fin( true ) );
		...
		// test credit: Sasha Moore
		{
			...
			test.execute( Close {} );
			test.execute(
			  ExpectMessage {}.with_syn( true ).with_fin( true ).with_payload_size( 0 ).with_seqno( isn ) );
			test.execute( ExpectNoSegment {} );
		}
